...
def async_setup_platform(hass, config, async_add_devices, discovery_info=None):
    _LOGGER.warning('Passou async_setup_platform')
    """Set up the OpenALPR local platform."""

    _LOGGER.warning('Passou async_setup_platform %s %s %s %s',config[CONF_ALPR_BIN], '-c', config[CONF_REGION], '-')
    ...
    _LOGGER.warning('Passou async_setup_platform 6')
...
class ImageProcessingAlprEntity(ImageProcessingEntity):
    ...
    def process_plates(self, plates, vehicles):
        ...
        _LOGGER.warning('Passou process_plates')
    ...
    def async_process_plates(self, plates, vehicles):
        _LOGGER.warning('Passou async_process_plates')
        """Send event with new plates and store data.
...
class OpenAlprLocalEntity(ImageProcessingAlprEntity):
    ...
    def async_process_image(self, image):
        _LOGGER.warning('Passou async_process_image')
        ...
        _LOGGER.warning('Passou async_process_image 3 %s', stdout)

        while True:
            _LOGGER.warning('Passou async_process_image 4')
            
            line = stdout.readline()
            _LOGGER.warning('Passou async_process_image 5 %s', line)

            if not line:
                break

            new_plates = RE_ALPR_PLATE.search(line)
            _LOGGER.warning('Passou async_process_image 6 %s', new_plates)
            new_result = RE_ALPR_RESULT.search(line)

            _LOGGER.warning('Passou async_process_image 7 %s', new_result)
        ...
        _LOGGER.warning('Passou async_process_image 8')
