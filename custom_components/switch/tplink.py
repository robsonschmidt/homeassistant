...
def setup_platform(hass, config, add_devices, discovery_info=None):
    ...
    add_devices([SmartPlugSwitch(SmartPlug(host), name, leds_on,host)], True)


class SmartPlugSwitch(SwitchDevice):
    """Representation of a TPLink Smart Plug switch."""

    def __init__(self, smartplug, name, leds_on, host):
    ...
    def update(self):
        ...
        try:
            ...
            try:
                host = self._host
                request={"schedule":{"get_monthstat":{"month": datetime.datetime.now().month, "year":datetime.datetime.now().year}}}
                result = self.smartplug.protocol.query(host,request)
                #self._emeter_params["Monthstat"] = result['schedule']['get_monthstat']

                for month in result['schedule']['get_monthstat']['month_list']:   
                    self._emeter_params["monthly_runtime"] = month['time']

                request={"schedule":{"get_daystat":{"month": datetime.datetime.now().month, "year": datetime.datetime.now().year}}}
                result = self.smartplug.protocol.query(host,request)
                #self._emeter_params["DayStat"] = result['schedule']['get_daystat']

                for day in result['schedule']['get_daystat']['day_list']:   
                    self._emeter_params["daily_runtime"] = day['time']
